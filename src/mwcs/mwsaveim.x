...
begin
	...
	do axis = 1, ndim {
		...
		call strcpy ("LINEAR  ", ctype, SZ_CTYPE)
		ifnoerr {
		    call mw_gwattrs (mw, axis, "label", label, SZ_VALSTR)
		} then {
		    call strupr (label)
		    if (nowhite (label, label, SZ_VALSTR) <= SZ_CTYPE) {
			if (strncmp (label, "SAMPLED", 8) != 0 &&
			    ...
			    call sprintf (ctype, SZ_CTYPE, "%-8s%9t")
				call pargstr (label)
			}
		    }
		}
		...
		if (and (FN_FLAGS(fn), F_RADEC) != 0) {
			...
			if (streq (axtype, "ra") || streq (axtype, "dec")) {
			    call strcpy ("DEC-----", ctype, SZ_CTYPE)
			} else if (streq (axtype[2], "lon") ||
				streq (axtype[2], "lat")) {
			    call sprintf (ctype, SZ_CTYPE, "%cLAT----")
				call pargc (axtype[1])
			} else {
			    call strcpy ("DEC-----", ctype, SZ_CTYPE)
			}
			...
			if (streq (axtype, "ra") || streq (axtype, "dec")) {
			    call strcpy ("RA------", ctype, SZ_CTYPE)
			} else if (streq (axtype[2], "lon") ||
				streq (axtype[2], "lat")) {
			    call sprintf (ctype, SZ_CTYPE, "%cLON----")
				call pargc (axtype[1])
			} else {
			    call strcpy ("RA------", ctype, SZ_CTYPE)
			}
		    ...
		    op = max (1, SZ_CTYPE - strlen (FN_NAME(fn)) + 1)
		    call strcpy (FN_NAME(fn), ctype[op], SZ_CTYPE-op+1)
		    call strupr (ctype)

		} else if (streq (FN_NAME(fn), "pc")) {
		    call mw_gwattrs (mw, axis, "axtype", axtype, 20)
		    call mw_gwattrs (mw, axis, "pc", kwname, SZ_KWNAME)
		    call sprintf (ctype, SZ_CTYPE, "%s-%s")
			call pargstr (axtype)
			call pargstr (kwname)
		    call strupr (ctype)
		    ...
		    call strcpy ("        ", ctype, SZ_CTYPE)
		    call strcpy (FN_NAME(fn), ctype, SZ_CTYPE)
		    call strupr (ctype)
		}
	    }
...
ewcs_
	...
	# Output any PV coefficients.
	do axis = 1, ndim {
	    npts = WCS_NPVC(wp,axis)
	    if (npts == 0)
		next
	    ...
	    do i = 0, npts-1 {
		dval = D(mw,pv+i)
		if (fp_equald(dval,0.0D0))
		    next
		cp = iw_findcard (iw, TY_PV, axis, i)
		call sprintf (kwname, SZ_KWNAME, "PV%d_%d")
		    call pargi (axis)
		    call pargi (i)
		if (cp == NULL)
		    call imaddf (im, kwname, "d")
		    ...
		    C_UPDATED(cp) = YES
		call imputd (im, kwname, dval)
	    }
	}

	# Output any PC coefficients.
	do axis = 1, ndim {
	    npts = WCS_NPC(wp,axis)
	    if (npts == 0)
		next
		...
		if (fp_equald(dval,0.0D0))
		    next
		cp = iw_findcard (iw, TY_PC, axis, i)
		call sprintf (kwname, SZ_KWNAME, "PC%d_%d")
		    call pargi (axis)
		    call pargi (i)
		if (cp == NULL)
		    call imaddf (im, kwname, "d")
