...
define	SPECTYPES  "|wave|freq|velo|vrad|vopt|zopt|"
...
double	theta, dval, maxval
char	ctype[8], prefix[4]
...
int	strncmp(), ctod(), strldxs(), strmatch(), strdic(), envgeti()
...
begin
	...
	do axis = 1, ndim {
	    ...
	    # Set prefix string.
	    if (ctype[5] == '-')
		call strcpy (ctype, prefix, 4)
	    else
		prefix[1] = EOS

	    # Determine the type of function on this axis.
	    j = strmatch (ctype, "-pc?$")
	    if (!IS_DIGIT(ctype[j-1]))
		j = 0
	    if (j > 0) {
		# PC corrections.
		ctype[j-4] = EOS
		...
		npc[i] = npc[i] + 1
		pc[npc[i],i] = axis
		call mw_swattrs (mw, axis, "axtype", ctype)
		call mw_swattrs (mw, axis, "pc", ctype[j-3])

	    } else if (strdic (prefix, Memc[str], SZ_LINE, SPECTYPES) > 0) {
		# Treat all as linear for now.
		call mw_swattrs (mw, axis, "label", ctype)

	    } else if (streq (ctype, "linear")) {
...
samperr_		call eprintf (
	    ...
	    # Set PV coefficients for this axis.
	    npts = IW_PVMAX(iw,axis)
	    if (npts > 0) {
		call calloc (pv, npts, TY_DOUBLE)
		do j = 0, npts-1 {
		    cp = iw_findcard (iw, TY_PV, axis, j)
		    if (cp != NULL) {
			rp = C_RP(cp)
			ip = IDB_STARTVALUE
		...
		call mw_spv (mw, axis, Memd[pv], npts)
		call mfree (pv, TY_DOUBLE)
	    }

	    # Set PC coefficients for this axis.
	    npts = IW_PCMAX(iw,axis)
	    if (npts > 0) {
		call calloc (pv, npts, TY_DOUBLE)
		do j = 0, npts-1 {
		    cp = iw_findcard (iw, TY_PC, axis, j)
		    if (cp != NULL) {
			rp = C_RP(cp)
			ip = IDB_STARTVALUE
			if (ctod (Memc[rp], ip, dval) > 0)
		...
		call mw_spc (mw, axis, Memd[pv], npts)
		call mfree (pv, TY_DOUBLE)
	    }
	}
	...
	do i = 1, 10 {
	    if (npc[i] > 0)
		call mw_swtype (mw, pc[1,i], npc[i], "pc", "")
	}
