...
pointer	s_map(), smw_openim()
int	get_next_number()
int	imtrgetim(), decode_ranges(), rv_verify_aps()
bool	streq()
errchk	s_map, swm_openim, rv_aplist, realloc, rv_verify_aps
...
int	rv_apmatch()
int	imtrgetim()
errchk	s_map, smw_openim, rv_aplist, realloc
...
begin
	...
	iferr (imo = s_map (Memc[oimage], READ_ONLY, 0)) {
	    call sfree (sp)
	    call rv_errmsg ("Object image does not exist.")
	    return (ERR_READ)
	}
	...
	iferr (imt = s_map (Memc[timage], READ_ONLY, 0)) {
	    call sfree (sp)
