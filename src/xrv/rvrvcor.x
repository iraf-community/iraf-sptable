...
begin
	...
	# Map the images.
	imo = s_map (IMAGE(rv), READ_ONLY, 0)
	imr = s_map (RIMAGE(rv), READ_ONLY, 0)
	...
	call s_unmap (imo)			# Free image pointers
	call s_unmap (imr)
	return (OK)
end
...
double	ut_start, ut_mid, int_time, time, s_getd()
int	code, flags, idx
char 	buf[SZ_LINE]

int	rv_parse_date(), rv_parse_timed(), s_accf(), stridx()
errchk 	s_getd()
...
begin
	...
	iferr (ep = s_getd (im, KW_EPOCH(rv))) {
	...
	if (s_accf(im,KW_UTMID(rv)) == YES) {
	    ...
	    if (idx == 0 || code == ERR_RVCOR) {
	        iferr (ut = s_getd (im, KW_UTMID(rv))) {
		    # Try to recover
	            if (rv_parse_timed (rv, im, is_obj, KW_UT(rv), ut_start) 
		        == ERR_RVCOR) {
	                    code = ERR_RVCOR
		    }
	            iferr (int_time = s_getd (im, KW_EXPTIME(rv))) {
	                call rv_err_comment (rv, 
	    ...
	    } else if (s_accf(im,KW_UT(rv)) == YES) {
...
begin
	iferr (call s_gstr(im, param, date, SZ_LINE)) {
	    if (is_obj == YES) {
	        call rv_err_comment (rv, 
...
begin
	...
	dval = INDEFD
        ...
        iferr (call s_gstr(im, param, Memc[buf], SZ_FNAME)) {
	    if (is_obj == YES) { 
	        call rv_err_comment (rv,
	    	    "ERROR: Error getting '%s' from object image header.",param)
	    } else {
