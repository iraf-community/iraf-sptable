...
begin
	im = s_map (IMAGE(rv), READ_WRITE, 0)
	if (S_NSPEC(im) > 1 && S_NDIM(im) > 1) {
	    ...
	    iferr {
		call s_addd (im, KW_HJD(rv), RV_HJD(rv))
		call s_addd (im, KW_MJD_OBS(rv), RV_MJD_OBS(rv))
		call s_addd (im, KW_VOBS(rv), RV_VOBS(rv))
		call s_addd (im, KW_VHELIO(rv), RV_VCOR(rv))
		} then
		    call rv_err_comment (rv, 
