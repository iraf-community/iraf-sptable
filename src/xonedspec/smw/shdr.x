...
double	w1, dw, z, shdr_lw()
bool	newim, streq()
pointer	sp, key, str, coeff, mw, ct, buf
pointer	smw_sctran(), s_gs3r(), un_open(), fun_open()
errchk	smw_sctran, s_gstr, imgeti, imgetr, smw_gwattrs
errchk	un_open, fun_open, fun_ctranr, s_gs3r, shdr_gtype
...
begin
	...
	if (sh == NULL) {
	    ...
	    call strcpy (S_FILE(im), Memc[str], SZ_LINE)
	...
	if (newim) {
	    call strcpy (S_FILE(im), IMNAME(sh), LEN_SHDRS)
	    IM(sh) = im
	    MW(sh) = smw
	    ...
	    iferr (call s_gstr (im, "DEREDDEN", RC(sh), LEN_SHDRS))
		RC(sh) = EOS
...
#call eprintf ("shdr_open: 50\n")
		...
		call sprintf (Memc[key], SZ_LINE, "cunit%d")
		    call pargi (i)
		iferr (call s_gstr (im, Memc[key], UNITS(sh), LEN_SHDRS)) {
	    ...
	    iferr (call s_gstr (im, "bunit", Memc[str], SZ_LINE))
...
#call eprintf ("shdr_open: 60\n")
	...
	case SMW_ND:
		...
		w1, dw, np2, z, APLOW(sh,1), APHIGH(sh,1), coeff) 
...
data_	# Set the coordinate and data arrays if desired otherwise free them.
	...
	case SMW_ND:
		...
		if (S_TBCOLS(im) != EOS)
		    call strcpy (S_TBCOLS(im), IMSEC(sh), LEN_SHDRS)
	    }
	case SMW_ES, SMW_MS:
	    if (mode == SHDATA || SY(sh) == NULL) {
		if (SY(sh) == NULL)
...
pointer	sp, key, str, im, smw, ct, buf, smw_sctran(), s_gs3r()
real	smw_c1tranr()
...
begin
	...
	do i = 1, 5 {
#call eprintf ("shdr_gtype: 10\n")
	    call sprintf (Memc[key], SZ_LINE, "BANDID%d")
		call pargi (i)
	    ifnoerr (call s_gstr (im, Memc[key], Memc[str], SZ_LINE)) {
		j = 1
	...
	buf = s_gs3r (im, NP1(sh), NP2(sh), i, i, j, j)
...
#call eprintf ("shdr_type: 10\n")
	call sprintf (Memc[key], SZ_FNAME, "BANDID%d")
	    call pargi (band)
	 iferr (call s_gstr (IM(sh), Memc[key], Memc[SID(sh,index)], LEN_SHDRS))
...
int	dummy, s_accf(), s_geti()

begin
	ival = default
	if (s_accf (im, field) == YES) {
	    iferr (dummy = s_geti (im, field))
		call erract (EA_WARN)
	    else
		ival = dummy
	}
