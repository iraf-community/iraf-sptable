...
int	s_geti(), mw_stati(), ctoi(), ctor(), ctod(), s_ofnl(), s_gnfn()
errchk	s_gstr, mw_gltermd, mw_sltermd
data	axes/1,2/

begin
#call eprintf ("smw_oldms\n")
	call smark (sp)
	...
	do i = 1, SMW_NSPEC(smw) {
		...
		call pargi (j)
	    call s_gstr (im, Memc[key], Memc[val], SZ_LINE)
	    call s_delf (im, Memc[key])
	...
	i = s_ofnl (im,
	    "DISPAXIS,APFORMAT,BEAM-NUM,DC-FLAG,W0,WPC,NP1,NP2", NO)
