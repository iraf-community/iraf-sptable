...
begin
#call eprintf ("smw_swattrs (%d, %d, %d, %d, %d, %d, %.4g, %.4g, %d, %.4g)\n")
#call pargi (smw)
#call pargi (index1)
#call pargi (index2)
#call pargi (ap)
...
start_
#call eprintf ("smw_swattrs: SMW_FORMAT = %d\n")
#call pargi (SMW_FORMAT(smw))
	...
	case SMW_ES:
#call eprintf ("smw_swattrs: 10\n")
	    # Check for any changes to the dispersion system.
	    # Non-linear must be multispec.
	    if (dtype == DCFUNC) {
		#call smw_esms (smw, dtype, w1, dw, nw, z)
		call smw_esms (smw)
		goto start_
	    }
#call eprintf ("smw_swattrs: 20\n")
...
#call eprintf ("smw_swattrs: 40 w1=%.4g\n")
#call pargd (w1)
	    SMW_DTYPE(smw) = dtype
	    SMW_NW(smw) = nw
	    SMW_W1(smw) = w1
...
#call eprintf ("smw_swattrs: 50\n")
	...
	case SMW_MS:
	    ...
	    # Check for consistency between dtype and coeff.
	    if (dtype == DCFUNC && coeff[1] == EOS)
	        call error (2, "function coefficients missing")
...
#call eprintf ("smw_swattrs: mw_swattrs (mw, 2, %s, %s)\n")
