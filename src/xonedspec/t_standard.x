...
pointer	imtopenp(), rng_open(), s_map(), smw_openim(), un_open()
errchk	s_map, smw_openim, shdr_open, std_calib, get_airm, ext_load, obsimopen
...
begin
	...
	while (imtgetim (list, Memc[image], SZ_FNAME) != EOF) {
		...
		call s_unmap (IM(sh))
		ifnoerr (im = s_map (Memc[image], READ_WRITE, 0)) {
		    IM(sh) = im
		    call s_seti (IM(sh), IM_WHEADER, YES)
		    call s_addr (IM(sh), "exptime", IT(sh))
		} else {
		    im = s_map (Memc[image], READ_ONLY, 0)
		    IM(sh) = im
		}
	    }
