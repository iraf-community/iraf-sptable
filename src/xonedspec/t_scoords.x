...
bool	verbose, clgetb()
pointer	sp, spec, coords, values, im, tmp, s_map()

errchk	s_map, open, scoords
...
begin
	...
	while (imtgetim (speclist, Memc[spec], SZ_FNAME) != EOF) {
		...
		# Open the image.
		tmp = s_map (Memc[spec], READ_WRITE, 0); im = tmp

		# Get the coordinate values.
		tmp = open (Memc[coords], READ_ONLY, TEXT_FILE); fd = tmp
		call salloc (values, S_NDISP(im)+1, TY_DOUBLE)
		n = 0
		while (fscan(fd) != EOF) {
...
errchk	mw_open, s_mwsaves, stropen
