...
int	clgwrd(), imtgetim(), imtlen(), s_accf(), nscan(), strncmp(), ctor()
pointer	imtopenp(), rng_open(), s_map(), smw_openim(), s_gsr(), s_psr()
errchk	s_map, smw_openim, shdr_open, deredden, deredden1
...
begin
	...
	while (imtgetim (inlist, Memc[input], SZ_FNAME) != EOF) {
		...
		if (s_accf (in, "DEREDDEN") == YES) {
			...
			if (clgetb ("uncorrect")) {
			    call s_gstr (in, "DEREDDEN", Memc[temp], SZ_LINE)
		...
		call s_astr (out, "DEREDDEN", Memc[log])
		...
		do j = 1, S_NAUX(in) {
			...
			case SMW_ES, SMW_MS:
				...
				call amovkr (Memr[SY(sh)+SN(sh)-1],
				    Memr[outbuf+SN(sh)], S_NDISP(out)-SN(sh))
			}
		    }
		}
	    } then {
