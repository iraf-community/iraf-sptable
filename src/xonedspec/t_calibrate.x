...
errchk	s_map, smw_openim, shdr_open, s_gsr, s_psr
...
begin
	...
	while (imtgetim (inlist, Memc[input], SZ_FNAME) != EOF) {
	    ...
	    # Map the input image.
	    iferr (in = s_map (Memc[input], READ_ONLY, 0)) {
		call erract (EA_WARN)
		next
	    }
	    smw = smw_openim (in)
		...
		if (IS_INDEF(AM(sh))) {
			...
			call s_unmap (in)
			ifnoerr (in = s_map (Memc[input], READ_WRITE, 0)) {
			    IM(sh) = in
			    call s_seti (IM(sh), IM_WHEADER, YES)
			    call s_addr (IM(sh), "airmass", AM(sh))
			} else {
			    in = s_map (Memc[input], READ_ONLY, 0)
			    IM(sh) = in
			}
		    }
		}
		...
		if (IS_INDEF (IT(sh)) || IT(sh) <= 0.) {
			...
			IM(sh) = in
			call s_seti (IM(sh), IM_WHEADER, YES)
			call s_addr (IM(sh), "exptime", IT(sh))
			call s_addr (out, "exptime", IT(sh))
		    } else {
		        in = s_map (Memc[input], READ_ONLY, 0)
			IM(sh) = in
		    }
	    ...
	    call s_addr (out, "AIRMASS", AM(sh))
	    call s_addi (out, "EX-FLAG", EC(sh))
	    call s_addi (out, "CA-FLAG", FC(sh))
	    if (doflux) {
		if (fnu)
		    call s_astr (out, "BUNIT", "erg/cm2/s/Hz")
		else
		    call s_astr (out, "BUNIT", "erg/cm2/s/A")
	    }
...
int	i, j, n, clgwrd()
pointer	sp, fname, im, smw, x, y, s_map(), smw_openim()
errchk	s_map, smw_openim, shdr_open, asifit
...
begin
	...
	im = s_map (Memc[fname], READ_ONLY, 0)
	smw = smw_openim (im)
	cal = NULL
