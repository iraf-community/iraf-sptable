...
pointer	im, im1, smw, smw1, ap, s_map(), smw_openim()
bool	clgetb()
real	clgetr()
errchk	open, s_map, smw_openim, dc_gms, dc_gec, dc_multispec, dc_echelle
...
begin
	...
	while (imtgetim (in, Memc[input], SZ_FNAME) != EOF) {
		...
		i = s_map (Memc[input], READ_ONLY, 0); im = i
		...
		case SMW_ND:
		    ...
		    im1 = s_map (Memc[str], READ_ONLY, 0)
		    smw1 = smw_openim (im1)
...
pointer	s_map(), s_gs3r(), s_ps3r(), mw_open(), smw_sctran()
bool	clgetb(), streq()
errchk	s_map, mw_open, smw_open, dispcora, s_gs3r, s_ps3r
...
begin
	...
	if (streq (input, output)) {
	    ...
	    out = s_map (Memc[temp], NEW_COPY, in)
	    if (S_PIXTYPE(out) != TY_DOUBLE)
		S_PIXTYPE(out) = TY_REAL
	} else {
	    out = s_map (output, NEW_COPY, in)
	    if (S_PIXTYPE(out) != TY_DOUBLE)
		S_PIXTYPE(out) = TY_REAL
	}
 ...
 call eprintf ("40\n")
	...
	do j = 1, n3 {
		...
		switch (dispaxis) {
		case 1:
		    indata = s_gs3r (in, 1, nin, i, i, j, j)
		    outdata = s_ps3r (out, 1, n1, i, i, j, j)
		case 2:
		    indata = s_gs3r (in, i, i, 1, nin, j, j)
		    outdata = s_ps3r (out, i, i, 1, n1, j, j)
		case 3:
		    indata = s_gs3r (in, i, i, j, j, 1, nin)
		    outdata = s_ps3r (out, i, i, j, j, 1, n1)
		}
...
bool	clgetb(), streq()
errchk	s_map, mw_open, smw_open, dispcora, s_gsr, s_psr
...
begin
	...
	if (!linearize) {
		...
		do j = 1, nb
		    do i = 1, nl
			call amovr (Memr[s_gsr(in,i,j)], Memr[s_psr(out,i,j)],
			    S_NDISP(in))
	    ...
	    S_NDISP(out) = DC_NW(ap,1)
	    do i = 2, nl
		S_NDISP(out) = max (DC_NW(ap,i), S_NDISP(out))
	    cti = smw_sctran (smw, "world", "logical", 3)
	    cto = smw_sctran (mwout, "logical", "world", 3)
	    do j = 1, nb {
		do i = 1, nl {
		    indata = s_gsr (in, i, j)
		    outdata = s_psr (out, i, j)
		    call aclrr (Memr[outdata], S_NDISP(out))
		    call dispcora (cti, i, cto, i, Memr[indata], nc,
			Memr[outdata], DC_NW(ap,i), flux, blank)
		    if (DC_NW(ap,i) < S_NDISP(out))
			call amovkr (Memr[outdata+DC_NW(ap,i)-1],
			    Memr[outdata+DC_NW(ap,i)],S_NDISP(out)-DC_NW(ap,i))
		}
	    }
	    call smw_ctfree (cti)
	    call smw_ctfree (cto)
...
pointer	sp, input, str, im, mw, ap, tbl, s_map(), smw_openim()
...
begin
	...
	while (imtgetim (in, Memc[input], SZ_FNAME) != EOF) {
	    iferr (im = s_map (Memc[input], READ_ONLY, 0))
		next
	    mw = smw_openim (im)
...
begin
	...
	do j = 1, 4 {
	    ...
	    ifnoerr (call s_gstr (im, Memc[refkey], Memc[refstr], SZ_LINE)) {
		call sprintf (Memc[dckey], SZ_FNAME, "DCLOG%d")
		    call pargi (i)
		call sprintf (Memc[dcstr], SZ_LINE, "%s = %s")
		    call pargstr (Memc[refkey])
		    call pargstr (Memc[refstr])
		call s_astr (im, Memc[dckey], Memc[dcstr])
		call s_delf (im, Memc[refkey])
		i = i + 1
	    }
	}
