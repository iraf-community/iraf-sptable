...
int	clgwrd(), imtgetim()
pointer	sp, ref, im, imtopenp(), s_map()
errchk	s_map
...
begin
	...
	if (clgetb ("assign")) {
	    im = s_map (image, READ_WRITE, 0)
	    if (wt1 < 0.9999995D0) {
	        call sprintf (Memc[str], SZ_LINE, "%s %.8g")
	            call pargstr (ref1)
		    call pargr (wt1)
	        call s_astr (im, "refspec1", Memc[str])
	        call sprintf (Memc[str], SZ_LINE, "%s %.8g")
	            call pargstr (ref2)
		    call pargr (wt2)
	        call s_astr (im, "refspec2", Memc[str])
	    } else {
	        call s_astr (im, "refspec1", ref1)
	        iferr (call s_delf (im, "refspec2"))
		    ;
	    }
	    call s_unmap (im)
	}
