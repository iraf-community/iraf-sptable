...
bool	un_compare()
double	smw_c1trand()
int	s_accf(), nscan(), stropen()
pointer	smw_sctran(), un_open()
errchk	dc_gmsdb, dc_refshft, s_gstr, smw_sctran, un_open
errchk	s_gstr
...
begin
	...
	if ((s_accf (im, "REFSPEC1") == NO) ||
	    (dc > -1 && s_accf (im, "DCLOG1") == NO)) {
	    if (fd1 != NULL) {
		call fprintf (fd1,
	...
	iferr {
	    call s_gstr (im, "REFSPEC1", Memc[str1], SZ_LINE)
	...
	iferr {
            call s_gstr (im, "REFSPEC2", Memc[str1], SZ_LINE)
            call sscan (Memc[str1])
            call gargwrd (Memc[str1], SZ_LINE)
            call gargd (wt2)
            if (nscan() == 1)
...
done_	# Set aperture parameters in terms of logical image.
	...
	do i = 1, naps {
	    ...
	    DC_NW(ap,i) = min (S_NDISP(im), max (j, k))
	}
	call smw_ctfree (ct1)
...
begin
	...
	call s_gstr (im, keywrd, Memc[refshft], SZ_FNAME)
...
int	s_accf(), nscan(), stropen()
pointer	smw_sctran(), un_open()
errchk	dc_gecdb, s_gstr, smw_sctran, un_open
...
begin
	...
	if (fd2 != NULL) {
        ...
        iferr {
            call s_gstr (im, "refshft1", Memc[str1], SZ_LINE)
	...
	iferr {
	    ...
	    iferr {
		call s_gstr (im, "refshft2", Memc[str1], SZ_LINE)
		call salloc (pshift3, naps, TY_DOUBLE)
		call salloc (pcoeff3, naps, TY_POINTER)
		call dc_gecdb (Memc[str1], stp, ap, un3, Memd[pshift3],
		    Memi[pcoeff3], naps, offset, slope)
