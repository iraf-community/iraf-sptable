...
pointer	imtopenp(), s_map(), smw_openim(), s_psr(), s_gsr()
errchk	s_map, smw_openim, shdr_open, twk_gcal, twk_tweak, s_psr, s_gsr
...
begin
	...
	while (imtgetim (inlist, Memc[input], SZ_FNAME) != EOF) {
		...
		do j = 2, S_NAUX(in) {
			...
			y = s_gsr (in, i, j)
			data = s_psr (out, i, j)
			call amovr (Memr[y], Memr[data], S_NDISP(out))
		    }
		}
	    } then {
		call erract (EA_WARN)
...
int	i, clgwrd()
pointer	sp, str, im, smw, s_map(), smw_openim()
real	clgetr()
