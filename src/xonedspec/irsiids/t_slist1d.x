...
int	btoi(), imtgetim(), s_geti()
bool	clgetb()
pointer	imtopenp(), s_map(), smw_openim()
errchk	s_map, smw_openim, shdr_open
...
begin
	...
	while (imtgetim (root, Memc[image], SZ_FNAME) != EOF) {
		...
		im = NULL
		mw = NULL
		ptr = s_map (Memc[image], READ_ONLY, 0); im = ptr
		...
		if (long_header == YES) {
		    ...
		    iferr (df = s_geti (im, "DF-FLAG"))
			df = -1
		    iferr (sm = s_geti (im, "SM-FLAG"))
			sm = -1
		    iferr (qf = s_geti (im, "QF-FLAG"))
			qf = -1
		    iferr (qd = s_geti (im, "QD-FLAG"))
			qd = -1
		    iferr (bs = s_geti (im, "BS-FLAG"))
			bs = -1
		    iferr (co = s_geti (im, "CO-FLAG"))
			co = -1
