...
pointer	rng_open(), s_map(), smw_openim(), s_psr()
errchk	open, s_map, smw_openim, shdr_open, s_getr
...
begin
	...
	while (imtgetim (ilist, Memc[input], SZ_FNAME) != EOF) {
		...
		do i = 1, SMW_NSPEC(mw) {
		    ...
		    if (Memc[scale] == '!')
			ST(shin) = s_getr (im, Memc[scale+1])
		    if (Memc[zero] == '!')
			HA(shin) = s_getr (im, Memc[zero+1])
		    if (Memc[weight] == '!')
			AM(shin) = s_getr (im, Memc[weight+1])
		    if (grdn)
			RA(shin) = s_getr (im, Memc[rdnoise])
		    if (ggain)
			DEC(shin) = s_getr (im, Memc[gain])
		    if (gsn)
			UT(shin) = s_getr (im, Memc[snoise])
		...
		# Write the results
		call amovr (Memr[SY(shout)], Memr[s_psr(im,j,1)], npts)
		if (nout != NULL)
		    call achtir (Memi[nc], Memr[s_psr(nout,j,1)], npts)
		call sfree (sp1)
	    }
...
pointer	s_map(), mw_open(), smw_openim()
...
begin
	...
	refim = s_map (IMNAME(SH(sh,1,1)), READ_ONLY, 0)
	im = s_map (output, NEW_COPY, refim)
	...
	if (naps == 1)
	    ...
	    S_NDIM(im) = 2
	call s_addi (im, "SMW_NDIM", S_NDIM(im))
	S_NSPEC(im) = naps
	if (S_PIXTYPE(im) != TY_DOUBLE)
	...
	if (noutput[1] != EOS) {
	    nout = s_map (noutput, NEW_COPY, im)
	    S_PIXTYPE(nout) = TY_INT
	    call sprintf (S_TITLE(nout), SZ_LINE, "Number combined for %s")
		call pargstr (output)
	}
