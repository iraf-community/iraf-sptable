...
pointer	rg_ranges(), s_map(), smw_openim()
...
begin
	...
	iferr {
		...
		if (imaccess (Memc[input], READ_ONLY) == YES) {
		    im = s_map (Memc[input], READ_ONLY, 0)
		    mw = smw_openim (im)
		...
		while (imtgetim (listout, Memc[output], SZ_FNAME) != EOF)
			...
			im = s_map (Memc[output], READ_ONLY, 0)
			mw = smw_openim (im)
			ymax = max (SMW_LLEN(mw,2), ymax)
			bmax = max (SMW_LLEN(mw,3), bmax)
			call smw_close (mw)
...
int	imaccess(), s_accf(), strcmp()
pointer	s_map(), smw_openim(), s_gsr(), s_psr()
...
begin
	...
	if (Memc[insect] != EOS || Memc[outsect] != EOS) {
		...
		do i = 1, S_NSPEC(in) {
		    b1 = s_gsr (in, i, j)
		    b2 = s_psr (out, i, j)
		    call amovr (Memr[b1], Memr[b2], S_NDISP(in, 1))
		}

	} else {

	    in = s_map (Memc[inroot], READ_ONLY, 0)
	    out = s_map (Memc[outroot], READ_WRITE, 0)

	    # Check dimensions.
	    if (S_NDISP(in) != S_NDISP(out) || S_NSPEC(in) != S_NSPEC(out) ||
		S_NAUX(in) != S_NAUX(out)) {
	...
	if (s_accf (in, SFT_KW) == YES)
	    call s_gstr (in, SFT_KW, Memc[inranges], SZ_LINE)
	else
	    call strcpy ("", Memc[inranges], SZ_LINE)

	if (s_accf (out, SFT_KW) == YES)
	    call s_gstr (out, SFT_KW, Memc[outranges], SZ_LINE)
	else {
	    call strcpy ("", Memc[outranges], SZ_LINE)
	    call s_astr (out, SFT_KW, Memc[outranges])
	}
	...
	if (s_accf (in, SFT_KWB) == YES)
	    call s_gstr (in, SFT_KWB, Memc[inranges], SZ_LINE)
	else
	    call strcpy ("", Memc[inranges], SZ_LINE)

	if (s_accf (out, SFT_KWB) == YES)
	    call s_gstr (out, SFT_KWB, Memc[outranges], SZ_LINE)
	else {
	    call strcpy ("", Memc[outranges], SZ_LINE)
	    call s_astr (out, SFT_KWB, Memc[outranges])
	}
...
begin
	...
	while (sft_getline (in, mw, sh, sf, gt, line, band) != EOF) {
		...
		case 1:
		    data = s_ps3r (out, 1, n, i, i, j, j)
		case 2:
		    data = s_ps3r (out, i, i, 1, n, j, j)
		case 3:
		    data = s_ps3r (out, i, i, j, j,  1, n)
		}
		if (SN(sh) < n)
...
begin
	...
	call s_gstr (im, SFT_KWB, Memc[linebuf], SZ_LINE)
	rg2 = rg_ranges (Memc[linebuf], 1, SMW_LLEN(mw,3))

	rgold = rg_union (rg1, rg2)
	call rg_encode (rgold, Memc[linebuf], SZ_LINE)
	call s_astr (im, SFT_KWB, Memc[linebuf])
