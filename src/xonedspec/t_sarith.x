...
double	shdr_lw()
int	imaccess(), ctod()
int	imtlen(), imtgetim(), s_gnfn()
bool	strne(), rng_elementi(), fp_equald()
pointer	s_map() , s_gsr(), s_psr(), s_ofnl()
pointer	smw_openim(), mw_open()
errchk	s_map, smw_openim, mw_open, s_unmap, s_gstr, imdelete
errchk	s_gsr, s_psr
...
begin
	...
	if (imaccess (output, READ_ONLY) == YES) {
		...
		ptr = s_map (output, READ_ONLY, 0); out = ptr
	...
	while (imtgetim (list1, Memc[input1], SZ_FNAME) != EOF) {
	    ...
	    ptr = s_map (Memc[input1], READ_ONLY, 0); in1 = ptr
	...
	if (out != NULL) {
	    ptr = s_map (Memc[temp], NEW_COPY, out); outtmp = ptr
	    if (S_PIXTYPE(outtmp) != TY_DOUBLE)
		S_PIXTYPE(outtmp) = TY_REAL

	    S_NDISP(outtmp) = max (npts, S_NDISP(out))
	    S_NSPEC(outtmp) = naps
	    S_NAUX(outtmp) = max (nbands, S_NAUX(out))
	    if (nbands > 1)
		S_NDIM(outtmp) = 3
	    else if (naps > 1)
		S_NDIM(outtmp) = 2
		...
		S_NDIM(outtmp) = 1
	    call s_setnew (outtmp)

	    l1 = 1.
	    dl = 1.
	    i = SMW_PDIM(MW(sh2))
	    j = SMW_PAXIS(MW(sh2),1)
	    mwoutdim = S_NDIM(outtmp)
	    ...
	    do j = 1, S_NAUX(out) {
		do i = 1, S_NSPEC(out) {
		    ptr = s_psr (outtmp, i, j)
		    call aclrr (Memr[ptr], S_NDISP(outtmp))
		    call amovr (Memr[s_gsr(out,i,j)], Memr[ptr], S_NDISP(out))
		    if (verbose) {
	    ...
	    do j = 1, S_NAUX(out)
		do i = S_NSPEC(out)+1, S_NSPEC(outtmp) {
		    ptr = s_psr (outtmp, i, j)
		    call aclrr (Memr[ptr], S_NDISP(outtmp))
		}
	    do j = S_NAUX(out)+1, nbands
		do i = 1, S_NSPEC(outtmp) {
		    ptr = s_psr (outtmp, i, j)
		    call aclrr (Memr[ptr], S_NDISP(outtmp))
		}
	    ...
	    if (nin > 1) {
		ptr = s_map (Memc[input2], READ_ONLY, 0); in1 = ptr
		ptr = smw_openim (in1); mwin1 = ptr
		call shdr_open (in1, mwin1, i, 1, INDEFI, SHDATA, sh1)
	    }
	    ptr = s_map (Memc[temp], NEW_COPY, in1); out = ptr
	    if (S_PIXTYPE(out) != TY_DOUBLE)
		S_PIXTYPE(out) = TY_REAL
	    ifnoerr (call s_gstr (out, "MSTITLE", Memc[str], SZ_LINE)) {
		call strcpy (Memc[str], S_TITLE(out), SZ_IMTITLE)
		call s_delf (out, "MSTITLE")
	    }
	    ...
	    j = s_ofnl (out, "DISPAXIS,APID*,BANDID*", NO)
	    while (s_gnfn (out, j, Memc[key], SZ_LINE) != EOF)
		call s_delf (out, Memc[key])
	    call s_cfnl (out, j)
	...
	while (imtgetim (list1, Memc[input1], SZ_FNAME) != EOF) {
	    ...
	    if (op > SQRT) {
		ifnoerr (ptr = s_map (Memc[input2], READ_ONLY, 0)) {
		    in2 = ptr
		    sh2 = NULL
		    mwin2 = NULL
		    ...
		    call malloc (const, S_NDISP(out), TY_REAL)
		    call amovkr (real (w), Memr[const], S_NDISP(out))
		}
	    }
		...
		do j = 1, SMW_NBANDS(mwin1) {
			...
			call sprintf (Memc[key], SZ_LINE, "BANDID%d")
			    call pargi (k)
			iferr (call s_gstr (out, Memc[key], Memc[str], SZ_LINE))
			    call s_astr (out, Memc[key], Memc[SID(sh1,1)])
			else {
			...
			Memr[SY(sh1)], Memr[s_psr(out,l,k)+NP1(sh1)-1],SN(sh1))
		...
		do j = k+1, S_NAUX(out)
		    call aclrr (Memr[s_psr(out,l,j)], S_NDISP(out))
	    }
	    } then
		call erract (EA_WARN)
...
pointer	s_map(), s_psr(), s_ofnl(), smw_openim(), mw_open()
errchk	s_map, smw_open, smw_openim, mw_open, s_unmap, s_psr, imdelete
errchk	shdr_open, sa_sextract
...
begin
	...
	while (imtgetim (list1, Memc[input1], SZ_FNAME) != EOF) {
		...
		ifnoerr (ptr = s_map (Memc[input2], READ_ONLY, 0)) {
		    ...
		    if (ctod (Memc[input2], i, w) <= 0)
			call error (1, "Error in second operand")
		    call malloc (const, S_NDISP(in1), TY_REAL)
		    call amovkr (real (w), Memr[const], S_NDISP(in1))
		}
	    }
		...
		do i = 1, SMW_NSPEC(mwin1) {
			...
			S_PIXTYPE(out) = TY_REAL
		    S_NDIM(out) = 1
		    if (!streq (TITLE(sh1), S_TITLE(out))) {
			call s_astr (out, "MSTITLE", S_TITLE(out))
			call strcpy (TITLE(sh1), S_TITLE(out),
			    SZ_IMTITLE)
		    }
		    j = s_ofnl (out, "DISPAXIS,APID*,BANDID*", NO)
		    while (s_gnfn (out, j, Memc[key], SZ_LINE) != EOF)
		    ...
		    if (Memc[SID(sh1,1)] != EOS)
			call s_astr (out, "BANDID1", Memc[SID(sh1,1)])
		    ...
		    p3 = (S_NDISP(out) - Memd[ltv2]) / Memd[ltm2]
		    nw = nint (min (max (p1, p3), max (p1, p2))) + NP1(sh1) - 1
		    ...
		    call sa_arith (op1, sh1, sh2, const, reverse,
			Memr[SY(sh1)], Memr[s_psr(out,1,1)+NP1(sh1)-1], SN(sh1))
