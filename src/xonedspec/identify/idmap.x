...
int	i, j, k, l, a, b, c, x1[3], x2[3], xs[3], s_len[3]
pointer	sp, wrd1, wrd2, wrd3, im

int	imaccess(), strdic(), ctoi(), nscan()
pointer	s_map()
errchk	s_map, id_maphdr
...
begin
	...
	im = s_map (ID_IMAGE(id), READ_ONLY, 0)
	...
	if (ID_SECTION(id) == EOS && S_NDIM(im) > 1) {
		...
		case FIRST:
		    ID_LINE(id,1) = 1
		case MIDDLE:
		    ID_LINE(id,1) = (1 + s_len[j]) / 2
		case LAST:
		    ID_LINE(id,1) = s_len[j]
		default:
		    l = 1
		...
		case FIRST:
		    ID_LINE(id,2) = 1
		case MIDDLE:
		    ID_LINE(id,2) = (1 + s_len[k]) / 2
		case LAST:
		    ID_LINE(id,2) = s_len[k]
		default:
		    l = 1
	...
	x1[1] = 1; x2[1] = s_len[1]; xs[1] = 1
	x1[2] = 1; x2[2] = s_len[2]; xs[2] = 1
	x1[3] = 1; x2[3] = s_len[3]; xs[3] = 1
	call id_section (ID_SECTION(id), x1, x2, xs, 3)

	# Set the axes.  The axis to be identified is the longest one.
	i = 1
	if (S_NDIM(im) > 1 && abs (x1[2]-x2[2]) >= abs (x1[i]-x2[i]))
	    i = 2
	if (S_NDIM(im) > 2 && abs (x1[3]-x2[3]) >= abs (x1[i]-x2[i]))
	    i = 3
	if (S_NDIM(im) > 3)
	    call error (1, "Image dimension greater than 3 not allowed")
	...
	ID_MAXLINE(id,1) = s_len[j]
	ID_MAXLINE(id,2) = s_len[k]
	...
	call id_maphdr (id, im, s_len)

	# In order to add REFSPEC the file has to be READ_WRITE.

	call s_unmap (im)
	iferr (im = s_map (ID_IMAGE(id), READ_WRITE, 0))
	    im = s_map (ID_IMAGE(id), READ_ONLY, 0)
...
procedure id_maphdr (id, im, s_len)

pointer	id				# ID pointer
pointer	im				# IMIO pointer
int	s_len[3]			# Axes lengths
...
begin
	...
	if (SMW_FORMAT(mw) != SMW_ND) {
	    ID_MAXLINE(id,1) = S_NSPEC(im)
	    ID_MAXLINE(id,2) = S_NAUX(im)
	    ID_NSUM(id,1) = 1
	    ID_NSUM(id,2) = 1
	...
	ID_NPTS(id) = s_len[SMW_LAXIS(mw,1)]
