...
pointer s_map(), smw_openim(), s_gsr(), s_psr(), s_ps3r()
errchk	s_map,  s_gsr, s_psr, s_ps3r, s_delf, shdr_open, wrspect1
errchk	smw_openim, smw_gwattrs, smw_swattrs, smw_saveim

begin
#call eprintf ("wrspect\n")
	in = IM(sh1)
	mw1 = MW(sh1)
	out = NULL
	mw2 = NULL
	...
	iferr {
		...
		call s_unmap (in)
		iferr (im = s_map (IMNAME(sh1), READ_WRITE, 0)) {
		    in = s_map (IMNAME(sh1), READ_ONLY, 0)
		...
		iferr (im = s_map (output, NEW_COPY, in)) {
		    if (!overwrite)
		        call erract (EA_ERROR)
		    im = s_map (output, READ_WRITE, 0); out = im

		    if (S_NSPEC(out) == 1) {
			call s_unmap (out)
			call imdelete (output)
			im = s_map (output, NEW_COPY, in); out = im
			if (S_PIXTYPE(out) != TY_DOUBLE)
			    S_PIXTYPE(out) = TY_REAL
			do j = 1, S_NAUX(out)
			    do i = 1, S_NSPEC(out)
				call amovr (Memr[s_gsr(in,i,j)],
				    Memr[s_psr(out,i,j)], S_NDISP(out))
		    }
			...
			if (S_NDIM(in) != S_NDIM(out))
			    call error (2, "Incompatible dimensions")
			if (S_NDISP(in) != S_NDISP(out) ||
			    S_NSPEC(in) != S_NSPEC(out) ||
			    S_NAUX(in) != S_NAUX(out))
				call error (3, "Incompatible dimensions")
			coeff = NULL
	    ...
	    if (EC(sh1) == ECYES)
		call s_addi (out, "EX-FLAG", EC(sh1))
	    else if (s_accf (out, "EX-FLAG") == YES)
		call s_delf (out, "EX-FLAG")
	    if (FC(sh1) == FCYES)
		call s_addi (out, "CA-FLAG", FC(sh1))
	    else if (s_accf (out, "CA-FLAG") == YES)
		call s_delf (out, "CA-FLAG")
	    if (RC(sh1) != EOS)
		call s_astr (out, "DEREDDEN", RC(sh1))
	    else if (s_accf (out, "DEREDDEN") == YES)
		call s_delf (out, "DEREDDEN")
		...
		case 1:
		    outbuf = s_ps3r (out, np1, np2, i, i, j, j)
		case 2:
		    outbuf = s_ps3r (out, i, i, np1, np2, j, j)
		case 3:
		    outbuf = s_ps3r (out, i, i, j, j, np1, np2)
		}
